...
parser.add_argument('--src_dir', default='E:/Rain13k/train', type=str, help='Directory for full resolution images')
parser.add_argument('--tar_dir', default='E:/Rain13k/train_crop_2',type=str, help='Directory for image patches')
...
def save_files(i):
    ...
    NUM_PATCHES = (H//PS + 1) * (W//PS + 1)

    if (H or W) < PS:
        cv2.imwrite(os.path.join(noisy_patchDir, '{}_0.png'.format(i + 1)), noisy_img)
        cv2.imwrite(os.path.join(clean_patchDir, '{}_0.png'.format(i + 1)), clean_img)
    else:
