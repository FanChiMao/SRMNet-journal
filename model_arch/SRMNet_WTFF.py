...
##--------------Wavelet thresholding feature fusion (WTFF)------------------
class WTFF(nn.Module):
    def __init__(self, in_channels, reduction=8, bias=True):
        super(WTFF, self).__init__()

        d = max(int(in_channels / reduction), 4)

        # thresholding
        self.thresholding = nn.Sequential(
            nn.Conv2d(3*in_channels, d, kernel_size=1, stride=1, padding=0, bias=bias), # except sub-band of LL
            nn.ReLU(),
            nn.Conv2d(d, 3*in_channels, kernel_size=1, stride=1, padding=0, bias=bias)
        )
        self.dwt = DWT(exceptLL=True)
        self.iwt = IWT()

    def forward(self, inp_feats):
        t_list = torch.stack(inp_feats)
        summary = sum(t_list)
        wavelet_in, LL = self.dwt(summary)  # before thresholding
        wavelet_out = self.thresholding(wavelet_in)  # after thresholding (without LL)
        wavelet_out = torch.cat([LL, wavelet_out], dim=1)
        result = self.iwt(wavelet_out)
...
##---------- SRMNet_WTFF ----------
class SRMNet_WTFF(nn.Module):
    def __init__(self, in_chn=3, wf=96, depth=4):
        super(SRMNet_WTFF, self).__init__()
        self.depth = depth
        ...
        self.final_ff = WTFF(in_channels=wf)
...
if __name__ == "__main__":
    ...
    model = SRMNet_WTFF(in_chn=3, wf=64, depth=4).cuda()
    out = model(input)
